...
export default function AuthShell({
  ...
  return (
    <div className="page-wash relative min-h-screen overflow-hidden">
      ...
      <Link
        to="/"
        className="absolute left-4 top-4 z-20 inline-flex items-center gap-2 rounded-full border border-[#d7e4df] bg-white/88 px-4 py-2 text-sm font-medium text-slate-700 shadow-sm backdrop-blur-md transition-all duration-200 hover:border-[#b8d7cf] hover:bg-white hover:text-[#0b7a71] sm:left-6 sm:top-6"
      >
            ...
            <div className="mt-10 grid gap-4 sm:grid-cols-3">
              {showcaseItems.map(({ icon: Icon, title: itemTitle, copy }) => (
                <div key={itemTitle} className="surface-card rounded-[28px] p-5">
                  <div className="inline-flex h-11 w-11 items-center justify-center rounded-2xl bg-[#e9f7f3] text-[#0b7a71]">
